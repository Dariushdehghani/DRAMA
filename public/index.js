...
socket.on('signed_in', data => {
    let json_data = JSON.stringify({username: data.username, email: data.email, password: data.password, key1: data.key1, key3: data.key3})
    localStorage.clear()
    ...
    document.getElementById('first_key_context').innerHTML = data.key1
    document.getElementById('third_key_context').innerHTML = data.key3
})
